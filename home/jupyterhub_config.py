...
def pre_spawn_hook(spawner):
    ...
    try:
        ...
        subprocess.check_call(['useradd', '-ms', '/bin/bash', '-g', 'mbda', username])
        subprocess.run(['chmod', '-077', '/home/'+username])
        subprocess.run(['cp', '-r', '/home/example_predef', '/home/'+username])
        subprocess.run(['chown', '-R',  username+':mbda', '/home/'+username+'/example_predef'])
        subprocess.run(['ln', '-s',  '/MEDA-Toolbox', '/home/'+username+'/MEDA-Toolbox'])
        subprocess.run(['ln', '-s',  '/FCParser', '/home/'+username+'/FCParser'])
        subprocess.run(['ln', '-s',  '/UGR-16', '/home/'+username+'/UGR-16'])
...
c.Authenticator.admin_users = {'codaslab'}
